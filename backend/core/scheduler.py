"""Background scheduler for HKO Weather Trader."""
import asyncio
...
async def weather_scan_and_trade_job():
    ...
    Runs every 15 minutes (configurable via WEATHER_SCAN_INTERVAL_SECONDS).
    """
    if kill_switch_active():
        log_event("warning", "Kill switch active — skipping scan")
        return
    ...
    try:
        ...
        try:
            ...
            if weather_pending >= MAX_WEATHER_ALLOCATION:
                log_event("info", f"Weather allocation limit: ${weather_pending:.0f}/${MAX_WEATHER_ALLOCATION:.0f}")
                return
    ...
    try:
        ...
        try:
            ...
            if settled:
                ...
                log_event("success", f"Settled {len(settled)}: {wins}W/{losses}L, P&L: ${total_pnl:.2f}")
            else:
...
async def heartbeat_job():
    """Periodic heartbeat. Runs every 5 minutes."""
    db = None
    try:
        db = SessionLocal()
        state = db.query(BotState).first()
        ...
        log_event("data", f"Heartbeat: {pending} pending, bankroll: ${state.bankroll:.2f}")
    except Exception as e:
...
def start_scheduler():
    ...
    settle_seconds = settings.WEATHER_SETTLEMENT_INTERVAL_SECONDS
    ...
    scheduler.start()
    log_event("success", "HKO Weather Trader scheduler started", {
